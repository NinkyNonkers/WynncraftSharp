...
public static class GuildLeaderboardExtensions
{
    ...
    {
        if (timeframe == -1) 
            return await client.GetAsync<LeaderboardGuilds>(false, new RequestParameter("timeframe", "alltime"));
        return await client.GetAsync<LeaderboardGuilds>(false, new RequestParameter("timeframe", timeframe.ToString()));
    }
}